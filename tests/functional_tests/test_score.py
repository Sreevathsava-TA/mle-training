...
@pytest.fixture
def model_path():
    # Path to the pretrained random forest model
...
def test_score_computation_accuracy(sample_data_path, model_path):
    ...
    # Calculate metrics with score_models function
    final_mse = mean_squared_error(y_test, predictions)
    final_rmse = np.sqrt(final_mse)

    # Assert metrics match
    assert np.isclose(final_mse, expected_mse, atol=1e-2)
    assert np.isclose(final_rmse, expected_rmse, atol=1e-2)
